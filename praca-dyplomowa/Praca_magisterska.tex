...
Detektor zastosowany przez autorów ma 24x24 piksele, co w przeliczeniu daje zbiór ponad 180000 cech różnych rodzajów i wymiarów. Same operacje arytmetyczne potrzebne do wyliczenia tych cech stanowić mogą spore wyzwanie (jeszcze większe stanowiły na początku lat 2000, gdy algorytm powstawał), dlatego autorzy opracowali metodę przedstawioną w kolejnej sekcji: wyliczanie formy pośredniej w postaci obrazu integralnego. 
...
gdzie $x$ to 24x24-pikselowe podokno obrazu, $h_{j}(x)$ - klasyfikator, $f_{j}$ - cecha, $\theta_{j}$ - próg i $p_{j}$ - znak liczby. Dokładniejszy opis zmodyfikowanego algorytmu AdaBoost znajduje się w algorytmie x. W praktyce żadna cecha nie jest w stanie w pojedynkę dokonać klasyfikacji z odpowiednio dużą dokładnością. W opisywanym przypadku współczynniki błędu cech wybranych w pierwszych rundach wynosiły między 0,1 a 0,3, później dochodząc do 0,4 i 0,5 (czyli aż do wyniku losowego rzutu monetą).

\subsection{Konstrukcja kaskady klasyfikatorów}

Wstępne eksperymenty autorów wykazały, że klasyfikator do wykrywania twarzy \textit{en face} (ang. \textit{frontal face classifier}), stworzony z 200 cech, miał współczynnik wykrycia twarzy na poziomie 95\%, jednocześnie popełniając błędy pierwszego rodzaju dla 1 na 14084 zdjęć. Taki klasyfikator potrzebował mniej niż sekundę na analizę obrazu wielkości 384 na 288 pikseli. Był to wynik dobry (szczególnie ze względu na szybkość działania), ale nie dostatecznie dobry do niektórych zastosowań praktycznych.
